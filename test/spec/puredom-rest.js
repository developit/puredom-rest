describe('puredom-rest', function() {
	it('should be a function', function() {
		expect(puredom.rest).to.exist;
		expect(puredom.rest).to.be.a.function;
	});

	it('should expose the Resource constructor', function() {
		expect(puredom.rest.Resource).to.be.a.function;
	});
	...
	it('should initialize with a URL', function() {
		...
		var api = puredom.rest(url);
		expect( api ).to.be.an.instanceof( puredom.rest.Resource );
		expect( api.url ).to.equal( url );
	});


	// patch for PhantomJS:
	window.ProgressEvent = function(type, props){ puredom.extend(this, props); };
	...
	before(function() {
		...
		server.respondWith('POST', multi, function(r) {
			reply(r, user(null, puredom.querystring.parse(r.requestBody)));
		});

		// Read
		server.respondWith('GET', single, function(r, id) {
			reply(r, user(id) || { success:false, message:'Not Found' });
		});
	...
	describe('#index()', function() {
		it('should issue a request to /', function(done) {
			var api = puredom.rest('/api/users');
	...
	describe('#get(id)', function() {
		...
		it('should return an error if status>=400', function(done) {
			...
			api.get('does-not-exist', function(err, user) {
				expect(err).to.deep.equal({ success:false, message:'Not Found' });
	...
	describe('#put([id, ] obj)', function() {
		it('should issue a JSON-encoded PUT request to /:id', function(done) {
			var api = puredom.rest('/api/users'),
				updatedUser = puredom.extend({}, users[0], {
					name : 'sheryll',
